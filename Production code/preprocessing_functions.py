...
def divide_train_test(df, target):
    # Let's separate into train and test set Remember to set
    # the seed (random_state for this sklearn function)
...
def impute_na(df, var, replacement='Missing'):
    # function replaces NA by value entered by user or
    # by string Missing (default behaviour)
    return df[var].fillna(replacement)

def elapsed_years(df, var, ref_var='YrSold'):
    # captures difference between a year variable and a reference
    # variable year in which the house was sold by default)
...
def remove_rare_labels(df, var, frequent_labels):
    # groups labels that are not in the frequent list into the
    # umbrella group Rare
